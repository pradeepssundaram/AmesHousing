...
amesHousing %>% slice_sample(n = 100)
```
...
The below snippet will neatly summarise the missing values by columns and its percentage.
```{r}
...
saveimages <- function(figpath="./figs/",imgname,plt=last_plot())
{
  ...
  ggsave(filename = fullname,plot = plt,device = png(),path=figpath)
...
plt<-ggplot(data = amesHousing) +
  ...
  rm(plt)
```
We will not visualize ID as it is not a predictor. We will start with visualizing numerical columns to undertsand the distribution.
```{r}
amesHousing %>% select(where(is.numeric)) %>% select(-SalePrice)
```
```{r}
amesHousing %>% slice_sample(n=100)
```
...
```{r}
setstandards <- function(plt)
{
  plt<-plt + theme_classic() + theme(plot.title = element_text(hjust = 0.5) ) +
...
For some we will plot the distribution and for some we will plot the counts.
```{r}
# Lot Frontage

plt<-ggplot(data=amesHousing ) +
  geom_histogram(mapping = aes(x = LotFrontage),alpha=.25,binwidth = 5, fill="#ffa600") +
  geom_density(mapping = aes(x=LotFrontage,y=5*..count..), color="#003f5c")+
  xlab("Lot Frontage in Feet") +
  ylab("Count of Houses")+
  ggtitle("Distribution of Lot Frontage")
plt<-setstandards(plt = plt)
plt
saveimages(imgname = "Dist-LotFrontage",plt=plt)
rm(plt)  
```
...
```{r}
plt<-ggplot(data=amesHousing ) +
  geom_histogram(mapping = aes(x = LotArea),alpha=.25,binwidth = 200, fill="#ffa600") +
  geom_density(mapping = aes(x=LotArea,y=200*..count..), color="#73a2c6")+
  xlab("Lot Frontage in Sq.Feet") +
  ylab("Count of Houses")+
  ggtitle("Distribution of Lot Area") 
plt<-setstandards(plt)
plt
saveimages(imgname = "Dist-LotArea",plt=plt)
rm(plt)
  
```
For year built we should just get the count of houses so a bar chart for that would help to determine how many houses were built in each year.
```{r}
...
numericcols<-colnames(amesHousing)[ !colnames(amesHousing)  
                       %in% 
                      union(factor_cols,c("Id","YearBuilt","YearRemodAdd","GarageYrBlt","MoSold","YrSold","BsmtFullBath"
...
ctitles <- c("Lot Frontage","Lot Area","Masonry Veneer Area","Basement Finished Area(Type 1)","Basement Finished Area(Type 2)"
             ...
             ,"3 Season Porch Area","Screened Porch Area","Area of the Pool","Value of Miscellaneous Items"
             )
...
#pmap(.l = list(xvar=numericcols,title=ctitles,axislabel=ctitles),.f = drawscatterplot)
# 
# ctitles <-c("MSSubClass",    "MSZoning",      "Street"  ,      "Alley",         "LotShape",      "LandContour",   "Utilities",     "LotConfig"    ,
...
# )
plots<-pmap(.l = list(xvar=factor_cols,title=factor_cols,axislabel=factor_cols),.f = drawboxplot)
#plot_grid(plotlist = plots,ncol = 2)
plots

```
Time for some transformations. The year* variables are treated as intergers or factors. A better idea wold be to actually  get the age of the house and time since remodelling.
```{r}
# amesHousing %>% 
